...
A Dagster + dbt Cloud orchestration project demonstrating multi-team data pipeline collaboration with three independent code locations.
...
│   ┌─────────────────────────────────────────────────────────────────┐   │
│   │  ┌──────────────────────────────────┐    ┌────────────────────┐ │   │
│   │  │ dbt_optimove/optimove/           │───▶│ downstream/        │ │   │
...
Each sub-project has its own `.env.example` file with the variables it needs:
...
**For individual code locations:**
```bash
# Ingestion team
cd dagster-poc/ingestion
cp .env.example .env
...
*Databricks:*
- `run_databricks_ingestion_job` - Triggers Databricks notebook job for data ingestion
- `run_databricks_transform_job` - Triggers Databricks notebook job for data transformation (depends on ingestion job)
...
**Output:** `dagster-poc/output/fact_virtual_count.json`

## Project Structure

```
dagster-poc/                                # Monorepo Root
...
| `DBT_CLOUD_MAX_CONCURRENT_RUNS` | Max concurrent dbt Cloud runs (default: 3) | dbt code location |
| `DBT_CLOUD_RETRY_FAILED` | Retry failed runs (default: false) | dbt code location |
...
| `DATABRICKS_USE_SERVERLESS` | Use serverless compute (default: `true`) |
...
1. **Ingestion** (`ingestion/` code location)
   ...
   - Optional: Databricks ingestion job (`run_databricks_ingestion_job`) is triggered
   - Optional: Databricks transform job (`run_databricks_transform_job`) runs after ingestion completes
...
# Sync dependencies for the project
kc sync -- dagster-poc
...
# Correct: export CSV_DATA_PATH=/home/user/projects/dagster-poc/data
```
...
- [dagster-poc/README.md](dagster-poc/README.md) - Detailed project documentation
- [dagster-poc/ingestion/README.md](dagster-poc/ingestion/README.md) - Detailed Sling configuration guide
- [dagster-poc/ingestion/.env.example](dagster-poc/ingestion/.env.example) - Ingestion environment variables
- [dagster-poc/dbt/README.md](dagster-poc/dbt/README.md) - dbt Cloud setup instructions
- [dagster-poc/dbt/.env.example](dagster-poc/dbt/.env.example) - dbt Cloud environment variables
- [dagster-poc/downstream/README.md](dagster-poc/downstream/README.md) - Downstream asset documentation
- [dagster-poc/downstream/.env.example](dagster-poc/downstream/.env.example) - Downstream environment variables
- [dagster-poc/SUMMARY.md](dagster-poc/SUMMARY.md) - Project summary and architecture notes
